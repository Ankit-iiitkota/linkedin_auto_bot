...
if tailor_resume_keywords:
    from modules.resumeTailor import generate_tailored_resume
...
# Function to get the resume to upload for this specific job, tailoring keywords/skills to the job description if enabled
def get_resume_for_job(job_id: str, job_description: str) -> str:
    '''
    Returns the path to the resume to upload for this job.
    If `tailor_resume_keywords = True` in settings.py, generates a keyword-tailored PDF for this job description
    (same template/layout, only ATS keyword wording adjusted). Falls back to `default_resume_path` on any failure.
    '''
    if not tailor_resume_keywords or job_description in (None, "Unknown"):
        return default_resume_path
    try:
        return generate_tailored_resume(job_description, job_id, ai_client=aiClient if use_AI else None)
    except Exception as e:
        critical_error_log("Failed to get tailored resume for job, using default resume.", e)
        return default_resume_path

# Function to answer common questions for Easy Apply
def answer_common_questions(label: str, answer: str) -> str:
    if 'sponsorship' in label or 'visa' in label: answer = require_visa
    elif 'month' in label and ('experience' in label or 'years' in label): answer = "0"
                        ...
                        try: 
                            try:
                                ...
                                while next_button:
                                    ...
                                    if useNewResume and not uploaded: uploaded, resume = upload_resume(modal, get_resume_for_job(job_id, description))
...
def main() -> None:
    ...
    try:
        ...
        quotes = choice([
            "Never quit. You're one step closer than before. - Ankit Chaurasiya", 
            "All the best with your future interviews, you've got this. - Ankit Chaurasiya", 
            "Keep up with the progress. You got this. - Ankit Chaurasiya", 
            "If you're tired, learn to take rest but never give up. - Ankit Chaurasiya",
